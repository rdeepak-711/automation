...
# When _top_highlights_wts cache is missing, WTS runs first to build it, then
# article metas re-run so top highlight cards get richer descriptions, then all
# 3 pages generate. When cache exists, all 3 pages generate in a single pass.
if prompt_step 22 "Generate L1 Pages" \
  ...
  if [[ "$_BEFORE" == "0" ]]; then
    echo "  → No _top_highlights_wts cache — running What to See first to build it..."
  ...
  echo "  → Generating all 3 L1 pages..."
